...
import { format, isPast, isToday, isTomorrow, compareAsc, differenceInSeconds } from 'date-fns';
import { Calendar, Clock, User, MapPin, Building, Home, Timer } from 'lucide-react';
...
export default function EmployeeAvailability() {
  ...
  const [countdownStatus, setCountdownStatus] = useState<"future" | "today" | "in-progress" | "none">("none");
  ...
  const updateCountdown = (now: Date) => {
    ...
    if (isToday(startDate)) {
      ...
      if (startDate > now) {
        ...
        if (hours > 0) {
          setCountdownText(`${hours} hour${hours > 1 ? 's' : ''} and ${minutes} minute${minutes > 1 ? 's' : ''}`);
        } else {
  ...
  const closestActivity = activities
    ...
    .sort((a, b) => compareAsc(parseDate(a.start_date), parseDate(b.start_date)))[0];
    ...
    <div className="rounded-lg bg-white shadow">
      ...
      <div className="bg-indigo-600 px-6 py-4 rounded-t-lg flex justify-between items-center shadow-md">
        <div className="flex items-center text-white">
          ...
          <span className="font-bold text-xl">{format(currentDateTime, "EEEE, MMMM d, yyyy")}</span>
        </div>
        <div className="flex items-center text-indigo-100 bg-indigo-700 px-3 py-1.5 rounded-md">
          <Clock size={16} className="mr-2" />
          <span className="font-medium">{format(currentDateTime, "h:mm a")}</span>
        </div>
      </div>
      ...
      <div className="p-5 pb-3 border-b border-gray-100">
        ...
        {closestActivity && countdownText && (
          <div className={`mt-3 flex items-center px-4 py-2 rounded-md border ${getCountdownStyle()}`}>
      ...
      <div className="p-5">
        {activities.length === 0 ? (
          <div className="text-center py-10 text-gray-500">
            ...
            <p className="text-sm text-gray-400">Employee is working as usual</p>
          </div>
        ) : (
          <>
            ...
            {closestActivity && (
              <div>
                <div className="text-sm font-medium text-indigo-600 mb-2">NEXT AGENDA</div>
                <div className="border-2 border-indigo-500 rounded-lg p-4 bg-indigo-50 relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getActivityTypeStyle(closestActivity.activity_type).bgColor} ${getActivityTypeStyle(closestActivity.activity_type).textColor}`}>
                      {getDurationDays(closestActivity.start_date, closestActivity.end_date)} day{getDurationDays(closestActivity.start_date, closestActivity.end_date) > 1 ? 's' : ''}
                    </span>
                  </div>
                  
                  {closestActivity.description && (
                    <p className="text-gray-600 mt-1">{closestActivity.description}</p>
                  )}
                  
                  <div className="mt-4 flex items-center text-sm text-gray-700">
                    <Calendar size={16} className="mr-1.5 text-gray-500" />
                    <span>{formatDateRange(closestActivity.start_date, closestActivity.end_date)}</span>
                  </div>
            ...
            <div className="mt-6">
              <div className="text-sm font-medium text-gray-500 mb-3">AGENDA TIMELINE</div>
              <div className="border-l-2 border-gray-200 ml-3 space-y-6 pl-8 relative">
                {activities
                  .filter(activity => activity.id !== closestActivity?.id)
                  .sort((a, b) => compareAsc(parseDate(a.start_date), parseDate(b.start_date)))
                  .map(activity => {
                    const typeStyle = getActivityTypeStyle(activity.activity_type);
                    
                    return (
                      <div key={activity.id} className="relative">
                        {/* Timeline dot */}
                        <div className={`absolute -left-[42px] mt-1.5 w-5 h-5 rounded-full ${typeStyle.bgColor} border-4 border-white shadow flex items-center justify-center`}>
                          {typeStyle.icon}
                        </div>
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow p-4">
                          <div className="flex justify-between items-start">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${typeStyle.bgColor} ${typeStyle.textColor}`}>
                              {getDurationDays(activity.start_date, activity.end_date)} day{getDurationDays(activity.start_date, activity.end_date) > 1 ? 's' : ''}
                            </span>
                          </div>
